...
import { Play, SortAsc, Layers, Filter } from 'lucide-react';
...
export const Guide: React.FC = () => {
  ...
  return (
    <div className="w-full h-full flex overflow-hidden bg-stone-900/40 md:rounded-[2rem] md:border border-stone-800 backdrop-blur-md shadow-2xl relative">
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col bg-stone-950/30 relative">
          
          {/* Responsive Header Wrapper */}
          <div className="flex flex-col bg-stone-900/50 backdrop-blur-md border-b border-stone-800 z-20">
              ...
              <div className="h-16 md:h-20 flex items-center justify-between px-4 md:px-8">
                  <div className="flex items-center">
                      ...
                      <span className="md:hidden text-[10px] font-mono bg-stone-800 text-stone-400 px-1.5 py-0.5 rounded">
              ...
              {/* Mobile Horizontal Category Pills - MOBILE ONLY */}
              <div className="md:hidden w-full overflow-x-auto no-scrollbar pb-3 px-4 flex items-center space-x-2">
                   {categories.map((cat) => (
                      <button
                        key={cat}
                        onClick={() => setActiveCategory(cat)}
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
              ...
              {groupedChannels ? (
                  Object.entries(groupedChannels).map(([group, groupChannels]) => (
                      <div key={group} className="mb-8 md:mb-10 animate-fade-in-up">
                          <h3 className="text-sm md:text-lg font-bold text-stone-400 mb-3 md:mb-4 flex items-center sticky top-0 bg-stone-950/80 backdrop-blur-sm p-2 rounded-lg md:bg-transparent md:p-0 z-10">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4 animate-fade-in-up">
...
const ChannelCard: React.FC<ChannelCardProps> = ({ channel, setChannel, setView }) => (
    ...
    >
        ...
        <div className={`absolute inset-0 flex items-center justify-center p-4 md:p-6 bg-gradient-to-br from-stone-800 to-stone-900`}>
             {channel.logo.startsWith('http') ? (
                 ...
                 <span className="text-2xl md:text-4xl font-black text-stone-700 group-hover:text-stone-500">{channel.logo}</span>
             )}
        </div>
        ...
        {/* Text Info */}
        <div className="absolute bottom-0 left-0 right-0 p-3 transform md:translate-y-1 md:group-hover:translate-y-0 transition-transform">
            <p className="text-[8px] md:text-[10px] font-bold text-orange-500 uppercase tracking-widest mb-0.5">{channel.category}</p>
            <h4 className="text-white font-bold leading-tight line-clamp-1 text-sm md:text-base">{channel.name}</h4>
        </div>

        {/* Play Icon - Mobile: Visible on active touch? Desktop: Hover */}
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/20 backdrop-blur-[1px]">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-white rounded-full flex items-center justify-center shadow-lg transform scale-50 group-hover:scale-100 transition-transform">
